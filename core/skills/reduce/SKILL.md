...
**Source attribution note:** /reduce always creates notes with singular `Source: [[X]]`. If the note is later enriched from a second source, /enrich converts this to a multi-source format:
```markdown
Sources:
...
**Multi-source notes:** When a note has been enriched from additional sources, the footer uses plural format:
```markdown
Sources:
- [[original-source]] -- original extraction
- [[second-source]] -- enriched with [what it added]
```
The YAML `source` field retains the original. The footer holds the full attribution chain. /enrich handles this conversion automatically.
