...
| Source attribution | Must have `Source: [[X]]` or `Sources:` list in footer | WARN |

**Source attribution check:**
- Single source: `Source: [[filename]]` — valid
- Multiple sources: `Sources:` with bullet list, each with context phrase — valid
- Each source entry must have a context phrase (not just `-- related`)
- If `Sources:` list exists, entries must include `-- original extraction` for the first entry
- Missing source attribution is WARN, not FAIL (some notes may be manually created)
