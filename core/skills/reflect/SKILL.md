...
**Reference note awareness:**

When connecting any note, actively search for relevant **reference notes** (category: person, tool, decision, open-question) that should be linked:

| Reference type | Search for | Link when |
|---------------|------------|-----------|
| `person` | People mentioned in the note's context or source | The person contributed to, recommended, or influenced this insight |
| `tool` | Tools, libraries, frameworks referenced | The tool enables, implements, or is relevant to this insight |
| `decision` | Related architectural or design choices | The decision constrains, enables, or motivated this insight |
| `open-question` | Questions this insight partially answers | The insight provides evidence toward (or against) the question |

Reference notes are first-class graph nodes. An insight about "structural code search beats regex" should link to `[[ast-grep supports structural search across 25 languages]]` and `[[john chen recommends ast-grep and prefers async]]` if those reference notes exist. When an agent later works on a related task, following these links surfaces the right tool and the right person to consult.
