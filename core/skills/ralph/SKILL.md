...
CRITICAL: Update the note's source attribution footer — convert singular
Source: [[X]] to plural Sources: list with context phrases for each source.
Detect structural issues (title drift, split candidates) and set post_enrich_action.
ONE PHASE ONLY. Do NOT run reflect.
```
